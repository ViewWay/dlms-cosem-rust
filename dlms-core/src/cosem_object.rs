...
mod tests {
    ...
    impl CosemObject for TestObject {
        ...
        fn logical_name(&self) -> ObisCode { self.name }
        fn attribute_count(&self) -> u8 { 3 }
        fn method_count(&self) -> u8 { 1 }
        fn attribute_to_bytes(&self, attr: u8) -> Option<Vec<u8>> {
            match attr {
    ...
    fn test_cosem_object_trait() {
        let obj = TestObject { name: ObisCode::CLOCK, value: 42 };
    ...
    fn test_attribute_roundtrip() {
        let mut obj = TestObject { name: ObisCode::CLOCK, value: 42 };
