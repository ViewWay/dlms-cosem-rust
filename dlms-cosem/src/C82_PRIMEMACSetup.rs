//! IC82 PrimeMacSetup
//! Blue Book Ed16: class_id=82, version=0
...
impl CosemObject for PrimeMacSetup {
    fn class_id(&self) -> u16 { 82 }
    ...
    fn attribute_to_bytes(&self, attr: u8) -> Option<Vec<u8>> {
        match attr {
            ...
            4 => Some(dlms_axdr::encode(&DlmsData::Boolean(self.mac_security_enabled))),
            _ => None,
        }
    }
    fn attribute_from_bytes(&mut self, attr: u8, data: &[u8]) -> Result<(), CosemObjectError> {
        match attr {
            ...
            4 => { let d=dlms_axdr::decode(data).map_err(|_|CosemObjectError::InvalidData)?; match d { DlmsData::Boolean(v) => { self.mac_security_enabled=v; Ok(()) } _ => Err(CosemObjectError::InvalidData) } }
...
mod tests {
    ...
    #[test]
    fn test_new() {
        let obj = PrimeMacSetup::new(ObisCode::new(0,0,0,0,0,0));
        assert_eq!(obj.class_id(), 82);
    }
    #[test]
    fn test_getter() {
        let obj = PrimeMacSetup::new(ObisCode::new(0,0,0,0,0,0));
