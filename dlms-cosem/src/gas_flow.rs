...
mod tests {
    ...
    #[test]
    fn test_gas_flow_value() {
        let gf = GasFlow::new(ObisCode::CLOCK, DlmsData::DoubleLong(42));
        assert_eq!(gf.value().as_i32(), Some(42));
    }

    #[test]
    fn test_gas_flow_roundtrip() {
        let mut gf = GasFlow::new(ObisCode::CLOCK, DlmsData::DoubleLong(0));
        let bytes = dlms_axdr::encode(&DlmsData::DoubleLong(500));
        gf.attribute_from_bytes(2, &bytes).unwrap();
        assert_eq!(gf.value().as_i32(), Some(500));
    }
}
