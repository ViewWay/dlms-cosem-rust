//! IC037 Single Phase Import

use dlms_core::{CosemObject, CosemObjectError, DlmsData, ObisCode};

/// Single Phase Import - measures imported energy on a single phase
pub struct SinglePhaseImport {
...
mod tests {
    ...
    #[test]
    fn test_single_phase_import_value() {
        let spi = SinglePhaseImport::new(ObisCode::ACTIVE_POWER_L1, DlmsData::DoubleLong(42));
        assert_eq!(spi.value().as_i32(), Some(42));
    }

    #[test]
    fn test_single_phase_import_roundtrip() {
        let mut spi = SinglePhaseImport::new(ObisCode::ACTIVE_POWER_L1, DlmsData::DoubleLong(0));
        let bytes = dlms_axdr::encode(&DlmsData::DoubleLong(99999));
        spi.attribute_from_bytes(2, &bytes).unwrap();
        assert_eq!(spi.value().as_i32(), Some(99999));
    }
}
