//! IC91 G3MacSetup
//! Blue Book Ed16: class_id=91, version=0
...
impl G3MacSetup {
    ...
    pub fn mac_tx_power(&self) -> u8 { self.mac_tx_power }
    pub fn set_mac_tx_power(&mut self, v: u8) { self.mac_tx_power = v; }
}

impl CosemObject for G3MacSetup {
    fn class_id(&self) -> u16 { 91 }
    ...
    fn attribute_to_bytes(&self, attr: u8) -> Option<Vec<u8>> {
        match attr {
            ...
            4 => Some(dlms_axdr::encode(&DlmsData::Unsigned(self.mac_tx_power))),
            _ => None,
        }
    }
    fn attribute_from_bytes(&mut self, attr: u8, data: &[u8]) -> Result<(), CosemObjectError> {
        match attr {
            ...
            4 => { let d=dlms_axdr::decode(data).map_err(|_|CosemObjectError::InvalidData)?; match d { DlmsData::Unsigned(v) => { self.mac_tx_power=v; Ok(()) } _ => Err(CosemObjectError::InvalidData) } }
...
mod tests {
    ...
    #[test]
    fn test_new() {
        let obj = G3MacSetup::new(ObisCode::new(0,0,0,0,0,0));
        assert_eq!(obj.class_id(), 91);
    }
    #[test]
    fn test_getter() {
        let obj = G3MacSetup::new(ObisCode::new(0,0,0,0,0,0));
        let _ = obj.mac_tx_power();
    }
}
