...
/// Water Flow - water flow measurement
...
mod tests {
    ...
    #[test]
    fn test_water_flow_value() {
        let wf = WaterFlow::new(ObisCode::CLOCK, DlmsData::DoubleLong(42));
        assert_eq!(wf.value().as_i32(), Some(42));
    }

    #[test]
    fn test_water_flow_roundtrip() {
        let mut wf = WaterFlow::new(ObisCode::CLOCK, DlmsData::DoubleLong(0));
        let bytes = dlms_axdr::encode(&DlmsData::DoubleLong(999));
        wf.attribute_from_bytes(2, &bytes).unwrap();
        assert_eq!(wf.value().as_i32(), Some(999));
    }
}
