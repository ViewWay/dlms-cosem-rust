...
pub struct DemandRegister {
    ...
    scaler: i8,
    period: u16,
    #[allow(dead_code)] number_of_periods: u8,
}

impl DemandRegister {
    pub fn new(logical_name: ObisCode, value: DlmsData) -> Self {
        Self { logical_name, current_value: value, unit: 0, scaler: 0, period: 60, number_of_periods: 1 }
    }

    pub fn value(&self) -> &DlmsData { &self.current_value }
    pub fn set_value(&mut self, value: DlmsData) { self.current_value = value; }
}

impl CosemObject for DemandRegister {
    fn class_id(&self) -> u16 { 5 }
    ...
    fn attribute_to_bytes(&self, attr: u8) -> Option<Vec<u8>> {
        match attr {
            ...
            4 => Some(dlms_axdr::encode(&DlmsData::DoubleLongUnsigned(self.period as u32))),
            _ => None,
        }
    }
    ...
    fn attribute_from_bytes(&mut self, attr: u8, data: &[u8]) -> Result<(), CosemObjectError> {
        match attr {
            2 => {
                self.current_value = dlms_axdr::decode(data).map_err(|_| CosemObjectError::InvalidData)?;
                Ok(())
            }
